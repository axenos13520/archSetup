(function hazy() {
    ...
    function setAccentColor(color) {
        ...
        try {
            fetch("http://127.0.0.1:31337/color", {
                method: "POST",
                mode: "no-cors",
                headers: {
            ...
            console.log("Failed to export accent color", e);
        }
    }
    ...
    async function fetchFadeTime() {
        try {
            ...
            const fadeTimeResponse =
                await Spicetify.Platform.PlayerAPI._prefs.get({
    ...
    function getCurrentBackground(replace) {
        ...
        if (toggles.UseCustomBackground || !url || !URL.canParse(url))
            return startImage;
        if (replace)
    ...
    async function onSongChange() {
        ...
        if (album_uri !== undefined && !album_uri.includes("spotify:show")) {
            ...
            Spicetify?.Player?.data?.item?.uri?.includes("spotify:episode")
        ) {
            // Podcast
    ...
    document.addEventListener("click", (event) => {
        if (event.target.closest(".main-entityHeader-topbarTitle"))
            scrollToTop();
    });
    ...
    function waitForElement(elements, func, timeout = 100) {
        const queries = elements.map((element) =>
    ...
    function updateLyricsPageProperties() {
        function setLyricsPageProperties() {
            function calculateLyricsMaxWidth(lyricsContentWrapper) {
                const lyricsContentContainer =
                    lyricsContentWrapper.parentElement;
                const marginLeft = Number.parseInt(
                    window.getComputedStyle(lyricsContentWrapper).marginLeft,
                    10,
                );
                const totalOffset =
                    lyricsContentWrapper.offsetLeft + marginLeft;
                return Math.round(
    ...
    function loadToggles() {
        ...
        toggles.UseCustomColor = JSON.parse(
  ...
  bannerInput.onchange = () => {
    ...
    const homeEdit = new Spicetify.Topbar.Button(
        "Hazy Settings",
        "edit",
        () => {
      ...
      <button class="hazyOptionToggle">
        <span class="toggleWrapper">
          <span class="toggle"></span>
                ...
                const isEnabled =
                    JSON.parse(localStorage.getItem(id)) ?? defVal;
                toggleRow
                    .querySelector(".toggle")
                    .classList.toggle("enabled", isEnabled);
      ...
      <div class="slider-container">
        ...
        <div class="slider-value">
          <p id="${id}-value" contenteditable="true" >${val}${end || "%"}</p>
                ...
                slider
                    ...
                    .addEventListener("input", () => {
                        ...
                        if (content.length > 3) {
                            ...
                            content = slider.querySelector(
                                `#${id}-value`,
                            ).textContent = content.slice(0, 3);
                        }
            ...
            srcInput.placeholder = "Background image URL";
            ...
            saveButton.onclick = async () => {
                ...
                toggleInfo.forEach((opt) =>
                    localStorage.setItem(
                        ...
                        document
                            .querySelector(
                                `.hazyOptionRow[name=${opt.id}] .toggle`,
                            )
